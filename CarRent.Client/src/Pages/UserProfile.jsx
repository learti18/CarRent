...
export default function UserProfile() {
  ...
  const activeRentals = rentals?.filter(r => r.status?.toLowerCase() === 'active')?.length || 0;
  const completedRentals = rentals?.filter(r => r.status?.toLowerCase() === 'completed')?.length || 0;
  const totalSpent = rentals?.reduce((sum, rental) => sum + (rental.amount || 0), 0).toFixed(2);
      ...
      <div className='max-w-6xl mx-auto'>
              ...
              <div className='flex flex-col md:flex-row gap-6 items-start md:items-end'>
                ...
                <div className='relative -mt-12'>
                  <div className='w-24 h-24 rounded-xl overflow-hidden border-4 border-white shadow-md'>
                    <img 
                      src={profileImageUrl ? `${IMGURL}${profileImageUrl}` : "/user.png"} 
                      alt={username || "User"} 
          ...
          <div className='space-y-6'>
            ...
            {isLoading ? (
              ...
              <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Unable to Load Your Rentals</h3>
                <p className="text-gray-600 mb-6 max-w-md mx-auto">{error.message || "We encountered an error while trying to load your rental history. Please try again later."}</p>
                <button 
              ...
              <div className="bg-white p-12 rounded-xl shadow-sm border border-gray-100 text-center">
                ...
                {statusFilter === 'all' ? (
                  <>
                    <h3 className="text-xl font-semibold text-gray-900 mb-3">No Rentals Yet</h3>
                    <p className="text-gray-600 mb-6 max-w-md mx-auto">You haven't rented any cars yet. Start your journey today with our premium selection of vehicles!</p>
                  </>
                ) : (
                  <>
                    <h3 className="text-xl font-semibold text-gray-900 mb-3">No {statusFilter} Rentals Found</h3>
                    <p className="text-gray-600 mb-6 max-w-md mx-auto">You don't have any rentals with the {statusFilter} status. Try changing your filter or rent a new car.</p>
                  </>
                )}
        ...
        {activeTab === 'favorites' && (
          <div className="bg-white p-12 rounded-xl shadow-sm border border-gray-100 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-3">Favorites Coming Soon</h3>
            <p className="text-gray-600 mb-6 max-w-md mx-auto">
              Soon you'll be able to save your favorite vehicles here for quick access when you want to rent them.
            </p>
            <button 
              onClick={() => setActiveTab('rentals')}
