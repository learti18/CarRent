...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full bg-white border-b z-[100]">
      <nav className="py-2 px-4 max-w-7xl mx-auto flex flex-row justify-between items-center relative">
        <Logo />
        <div className="ml-auto relative">
          <button
            className={`relative cursor-pointer flex items-center justify-center p-1 rounded-full
                       transition-all duration-200 ${
                         dropdownMenu ? "bg-gray-100" : "hover:bg-gray-50"
                       }`}
          ...
          >
            <div className="relative">
              <img
                src={
                  getCurrentUser()?.profileImageUrl
                    ? IMGURL + getCurrentUser().profileImageUrl
                    : "user.png"
                }
                alt="User avatar"
                lazy="true"
